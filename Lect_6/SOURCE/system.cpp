...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double h_eff = _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H;
    double prob_cond = 1./(1+exp(-2*_beta*h_eff));
    double rand = _rnd.Rannyu();
    if (rand<prob_cond) _particle(i).setspin(1);
...
// TO BE FIXED IN EXERCISE 6
if(_measure_cv) {
  // Salvo E^2 per poter calcolare la varianza successivamente
  double energy_temp2 = 0.0;
  for (int i = 0; i < _npart; i++) {
    double s_i = double(_particle(i).getspin());
    double s_j = double(_particle(this->pbc(i+1)).getspin());
    energy_temp2 += pow(- _J * s_i * s_j - 0.5 * _H * (s_i + s_j),2);
  }
  energy_temp2 /= double(_npart); // energia per particella
  _measurement(_index_cv) = energy_temp2;  // <E^2>
}
...
void System :: averages(int blk){
  ...
  if (_measure_cv) {
    double e_mean_block = _average(_index_tenergy);  // <E> del blocco
    double e2_mean_block = _average(_index_cv);      // <E^2> del blocco
  
    // Calcolo del Cv per il blocco
    _average(_index_cv) = _beta * _beta * (e2_mean_block - e_mean_block * e_mean_block);
  }
