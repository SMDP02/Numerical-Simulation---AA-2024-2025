...
int main (int argc, char *argv[]){
   ...
   // Chi-square test
   /****************************************************************/
   int M_chi = 100;            // Number of intervals in [0,1]
   int N_chi = 10000;          // Number of total throws
   int exp_counts = N/M;   // Number of expected counts in each interval
   int numtest = 100;      // Number of tests
   
   // Calculation
   /****************************************************************/
   vector<double> chi_squared_values;
   ofstream output_counts;
   output_counts.open("output_counts.data");
   for (int i = 0; i < numtest; i++) {
      vector<int> counts(M_chi, 0);
      for (int j = 0; j < N_chi; j++) {
         int index = static_cast<int>(rnd.Rannyu() * M_chi); // Index of the interval containing rnd.Rannyu()
         counts[index]++;
      }
   double chi_squared = chisquare(counts, exp_counts);
   chi_squared_values.push_back(chi_squared); 
   output_counts << counts[i] << endl;
   }
   output_counts.close();
   /****************************************************************/
   // Writing on output file
   /****************************************************************/
   ofstream output_chi;
   output_chi.open("output_chi.data");
   for (int i = 0; i < numtest; i++){
      output_chi << chi_squared_values[i] << endl;
   }
   output_chi.close();
   /****************************************************************/
