...
void System::initialize_from_file(Random& rnd, const string& filename, int ncities, int npop, int ngen, double mut_prob, double p_exp) { 
    ...
    if (!input.is_open()) {
        cerr << "Rank " << _generation_index << " Error: Cannot open " << filename << endl;
        exit(1);
    }

    double lon, lat;
    int i = 0;
    while (i < _ncities && input >> lon >> lat) {
        _positions(i, 0) = lon;
        _positions(i, 1) = lat;
        i++;
    }
    ...
    compute_distance_matrix(); // Distances are identical across all ranks

    for(int i=0; i<_npop; i++) { 
        Individual indiv(_ncities, _distance_matrix); 
        indiv.initialize(*_rnd); // Each rank starts with a different random population
        _individuals.push_back(indiv); 
    } 
    fitness_sorting();
