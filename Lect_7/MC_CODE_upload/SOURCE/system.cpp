  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4*sqrt(_temp) / _n_bins_v;
        _measure_pofv = true;
...
if (_measure_pofv) {
  vec pofv_temp;
  for (int i=0; i<_npart; i++) {
      double v_mod = norm(_particle(i).getvelocity());
      int bin = static_cast<int>(v_mod / _bin_size_v);
      if (bin < _n_bins_v) {
        pofv_temp(bin) += 1.;
      }
  }
  
  double norm_factor = _bin_size_v * _nsteps * _npart; // Normalization factor
  for (int b=0; b<_n_bins_v; b++) {
      double v = (b + 0.5) * _bin_size_v; // bin center
      double mb_theory = (1.0/pow(2.0*M_PI*_temp, 1.5)) * 4.0 * M_PI * v * v * exp(-(v*v)/(2.0*_temp));
      // Normalizing distribution
      _measurement(_index_pofv + b) = pofv_temp(b)/norm_factor;
  }
}
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
...
if (_measure_pofv) {
  ...
  for (int b=0; b<_n_bins_v; b++) {
      double v = (b + 0.5) * _bin_size_v;
      double mb_theory = (1.0/pow(2.0 * M_PI * _temp, 1.5)) * 4.0 * M_PI * v * v * exp(-(v*v)/(2.0*_temp));
      
      sum_average = _global_av(_index_pofv + b)/double(blk);
      sum_ave2 = _global_av2(_index_pofv + b)/double(blk);
  
      coutf << setw(12) << v 
            << setw(12) << sum_average 
            << setw(12) << mb_theory       
            << setw(12) << this->error(_global_av(_index_pofv + b), _global_av2(_index_pofv + b), blk) 
            << endl;
  }
  coutf.close();
}
