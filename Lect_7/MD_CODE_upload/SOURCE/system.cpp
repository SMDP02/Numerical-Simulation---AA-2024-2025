...
// Function performing a simulation step
...
void System::ReverseVerlet() {
  if (!_reverse) return;
    for (int i = 0; i < _npart; i++) {
      // Positions
      const double xold = _particle(i).getposition(0, false);
      const double yold = _particle(i).getposition(1, false);
      const double zold = _particle(i).getposition(2, false);

      const double xnew = _particle(i).getposition(0, true);
      const double ynew = _particle(i).getposition(1, true);
      const double znew = _particle(i).getposition(2, true);
      ...
      // Reverse velocity
      _particle(i).setvelocity(0, this->pbc(xold - xnew, 0) / (2.0 * _delta));
      _particle(i).setvelocity(1, this->pbc(yold - ynew, 1) / (2.0 * _delta));
      _particle(i).setvelocity(2, this->pbc(zold - znew, 2) / (2.0 * _delta));
    }
  _reverse = false;
}
...
double System :: Force(int i, int dim){
  ...
  for (int j=0; j<_npart; j++){
    if(i != j){
      ...
      if(dr < _r_cut){ // Only interactions occurring at distances r < r_cut are considered
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    if(cinf.is_open()){
      ...
      cinf >> comment;
      for(int i=0; i<_npart; i++){
        cinf >> particle >> xold >> yold >> zold; // Units of coordinates in conf-1.xyz are fractions of _side
  ...
  } else { // Implementation of a velocity distribution represented by a Dirac delta centered at v_T
    // Ensure 3D
    if(_ndim != 3){
      cerr << "PROBLEM: initialize_velocities expects _ndim == 3" << endl;
      exit(EXIT_FAILURE);
    }
    ...
    vec sumv(_ndim); sumv.zeros();
    double v_T = sqrt(3.0 * _temp);

    for (int i=0; i<_npart; i++){
      int comp = int(_rnd.Rannyu(0.0, 3.0)); // 0,1,2
      bool sign = (_rnd.Rannyu() < 0.5);
      double val = sign ? v_T : -v_T;
      if(comp == 0) vx(i) = val;
      else if(comp == 1) vy(i) = val;
      else vz(i) = val;
    ...
    sumv2 /= double(_npart); // <v^2>

    double scalef = 1.0;
    if (sumv2 > 0.0) {
      scalef = sqrt( 3.0 * _temp / sumv2 ); // Equipartition <v^2> = 3 T
      for (int i=0; i<_npart; i++){
        vx(i) *= scalef;
        vy(i) *= scalef;
        vz(i) *= scalef;
      ...
      cerr << "PROBLEM: zero velocity norm in initialize_velocities (sumv2 == 0)" << endl;
      exit(EXIT_FAILURE);
    }
    // set velocities in particles and compute old positions consistent with Verlet
    for (int i=0; i<_npart; i++){
      _particle(i).setvelocity(0, vx(i));
      _particle(i).setvelocity(1, vy(i));
      _particle(i).setvelocity(2, vz(i));

      xold = this->pbc( _particle(i).getposition(0,true) - vx(i)*_delta, 0);
      yold = this->pbc( _particle(i).getposition(1,true) - vy(i)*_delta, 1);
      zold = this->pbc( _particle(i).getposition(2,true) - vz(i)*_delta, 2);
  ...
  if (input.is_open()){
    while ( input >> property ){
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4*sqrt(_temp) / _n_bins_v; // Setting bin amplitude
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0)); // When starting the simulation, we have to multiply by _halfside, as we want the system to be contained in a box of side equal to _halfside
...
void System :: measure(){ // Measure properties
  ...
  for (int i=0; i<_npart-1; i++){
    for (int j=i+1; j<_npart; j++){
      ...
      distance(0) = this->pbc(
          _particle(i).getposition(0,true) - _particle(j).getposition(0,true), 0);
      distance(1) = this->pbc(
          _particle(i).getposition(1,true) - _particle(j).getposition(1,true), 1);
      distance(2) = this->pbc(
          _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);

      dr = sqrt(dot(distance,distance));

      // ===== g(r): SOLO ACCUMULO =====
      if (_measure_gofr && dr < _halfside.min()) {
        int i_bin = int(dr / _bin_size);
        if (i_bin < _n_bins)
          _measurement(_index_gofr + i_bin) += 2.0;
      }
  ...
  if (_measure_pofv){
    double v_x, v_y, v_z;
    for(int i = 0; i < _npart; i ++){
       v_x = _particle(i).getvelocity(0);
       v_y = _particle(i).getvelocity(1);
       v_z = _particle(i).getvelocity(2);
       // Compute velocity modulus for each particle
       double mod_v = sqrt(v_x*v_x + v_y*v_y + v_z*v_z); 
       // Check which bin the value falls into
        for (int i_bin = 0; i_bin < _n_bins_v; i_bin ++) {
          if (mod_v < (i_bin + 1) * _bin_size_v && mod_v >= i_bin * _bin_size_v) {
            _measurement(_index_pofv + i_bin)++; // Increment the counter
            break;
        }
      }
    }
    ...
    for(int i_bin = 0; i_bin < _n_bins_v; i_bin ++){
      double v_low = i_bin * _bin_size_v;
      double v_up = (i_bin + 1) * _bin_size_v;
      double bin_width = v_up - v_low;

      // Normalization on the total number of particles and bin amplitude
      _measurement(_index_pofv + i_bin) /= _npart;
      _measurement(_index_pofv + i_bin) /= bin_width;
    }
  }
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure) _measurement[_index_pressure] = _rho * (2.0/3.0) * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
  ...
  if(_measure_magnet or _measure_chi) {
    ...
    _measurement(_index_chi) = _beta*pow(magnetization, 2)/double(_npart);
  }
...
void System :: averages(int blk){
  ...
  _average     = _block_av / double(_nsteps); // Contains block means (mean on the number of measures done in each block)
  _global_av  += _average;                    // Collects the means till block blk (progressive means)
  _global_av2 += _average % _average;         // % -> >Element-wise multiplication: each component of the new vector is the product of the correspective components of the two multiplied vectors

  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/potential_energy.dat",ios::app);
    average  = _average(_index_penergy);
    sum_average = _global_av(_index_penergy); // Progressive
    sum_ave2 = _global_av2(_index_penergy);   // Quadratic progressive
    coutf << setw(15) << blk 
          << setw(15) << average
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){ // For each progressive sum, save the corresponding histogram
  coutf.open("../OUTPUT/GOFR.dat",ios::app);
  for(int i_bin = 0; i_bin < _n_bins; i_bin++){
    average  = _average(_index_gofr + i_bin);
    sum_average = _global_av(_index_gofr + i_bin);
    sum_ave2 = _global_av2(_index_gofr + i_bin);

    double r_1 = i_bin * _bin_size;
    double r_2 = (i_bin + 1) * _bin_size;
    double r = 0.5 * (r_1 + r_2);
    double deltaV = (4.0/3.0) * M_PI * (pow(r_2,3) - pow(r_1,3));

    double norm = _rho * _npart * deltaV;

    average      = _average(_index_gofr + i_bin) / norm;
    sum_average  = (_global_av(_index_gofr + i_bin) / double(blk)) / norm;
    sum_ave2     = (_global_av2(_index_gofr + i_bin) / double(blk)) / (norm*norm);

    coutf << setw(15) << r
      << setw(15) << average
      << setw(15) << sum_average
      << setw(15) << this->error(sum_average*blk, sum_ave2*blk, blk)
      << endl;
  ...
  // POFV //////////////////////////////////////////////////////////////////////
  if (_measure_pofv){ // For each progressive sum, save the corresponding histogram
    coutf.open("../OUTPUT/POFV.dat",ios::app);
    for(int i_bin = 0; i_bin < _n_bins_v; i_bin++){
    average  = _average(_index_pofv + i_bin);
    sum_average = _global_av(_index_pofv + i_bin);
    sum_ave2 = _global_av2(_index_pofv + i_bin);

    double v_low = i_bin * _bin_size_v;
    double v_up = (i_bin + 1) * _bin_size_v;
    double v = 0.5 * (v_low + v_up);
  ...
  coutf << setw(15) << blk << setw(15) << fraction << endl;
  coutf.close();
