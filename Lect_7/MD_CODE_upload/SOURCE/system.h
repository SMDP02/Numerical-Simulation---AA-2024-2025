...
// Field --> construction of vectors/matrices/tensors of other objects
// We use this as structure of many "Particle"
class System {

private:
  const int _ndim = 3;  // Dimensionality of the system
  bool _restart;        // Flag indicating if the simulation is restarted (0=from scratch, 1=restarting from a certain configuration)
  int _sim_type;        // Type of simulation 
  bool _reverse = true;   // Flag to reverse r_{n} and r_{n-1} when performing Reverse_Verlet
  ...
  int _n_bins_v;         // Number of bins for velocity modulus distribution (to produce p(v))
  double _bin_size;      // Size of bins for radial distribution function
  double _bin_size_v;    // Size of bins for velocity modulus distribution (to produce p(v))
  double _vtail, _ptail; // Tail corrections for energy and pressure
  // ACCUMULATORI delle VARIABILI
